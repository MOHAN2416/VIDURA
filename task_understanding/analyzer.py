...
CONSTRAINT_PATTERNS = [
    (r"\bwithout changing (?:existing|current) behavior\b", "preservation of existing behavior"),
    (r"\bwithout breaking (?:existing|current) (?:tests|behavior|functionality)\b", "preservation of existing tests and behavior"),
    (r"\bwithout modifying (?:existing|current) (?:behavior|code)\b", "preservation of existing behavior"),
    (r"\bpreserve (?:existing|current) behavior\b", "preservation of existing behavior"),
...
class DeveloperTaskAnalyzer:
    ...
    def _extract_explicit_symbols(self, text: str) -> list[str]:
        ...
        for m in matches:
            cleaned = m.rstrip(".,;:!?")
            if cleaned and cleaned not in ignored and cleaned in text and cleaned not in symbols:
    ...
    def _rule_based_classify(
        ...
        # Code change
        if "add " in lower_req or "create " in lower_req or "modify " in lower_req or "update " in lower_req or "implement " in lower_req or "refactor " in lower_req:
            return DeveloperTask(
